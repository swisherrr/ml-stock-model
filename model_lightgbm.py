...
def train_lightgbm_model(X_train, y_train, X_val=None, y_val=None,
    ...
    Args:
        ...
        max_depth: Maximum depth of trees (-1 means no limit)
...
def cross_validate_lgb(X, y, n_splits=5, n_estimators=200, max_depth=6,
    ...
    """
    print("Performing time-series cross-validation (LightGBM)...")
...
def train_and_evaluate_lgb(X, y, feature_names, test_size=0.2, n_splits=5,
                           n_estimators=200, max_depth=6, learning_rate=0.05,
                           num_leaves=31, random_state=42, save_model=True,
                           model_dir='models'):
    """
    Complete training and evaluation pipeline for LightGBM
    ...
    cv_results = cross_validate_lgb(X_train, y_train, n_splits=n_splits,
    ...
    final_model = train_lightgbm_model(
        X_train, y_train,
        class_weights=calculate_class_weights(y_train),
        n_estimators=n_estimators,
        max_depth=max_depth,
    ...
    print(f"\n{'='*60}")
    print("Final Test Set Evaluation (LightGBM)")
    print(f"{'='*60}")
    ...
    if save_model:
        ...
        model_path = os.path.join(model_dir, f'lightgbm_model_{timestamp}.pkl')
